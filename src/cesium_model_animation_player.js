...
export class AnimationPlayer {
  constructor(animation_set, entity, fps) {
    ...
    for (const node_name in this.animation_set.nodes) {
  ...
  setAnimation(animation_name) {
    for (let i = 0; i < this.animation_set.animations.length; i++) {
  ...
  play(animation_name) {
    ...
    const { animations } = this.animation_set;
  ...
  _update() {
    if (this.play_state === PLAY_STATE.PLAY) this.setTime(this.current_time + this._frame_duration * this.speed);
  }
  ...
  setTime(current_time) {
    ...
    for (const track_name in this.current_animation.tracks) {
      ...
      if (typeof curr_rot_keys !== 'undefined' && curr_rot_keys.length > 0) {
        ...
        if (curr_rot_keys[0].time == curr_rot_keys[1].time) {
          ...
          if (Math.abs(resultAngle) > Cesium.Math.EPSILON5) Cesium.Quaternion.computeAxis(result, resultAxis);
...
export class AnimationParser {
  ...
  static parseAnimationNodesFromArrayBuffer(array_buffer) {
    ...
    for (let i = 0; i < gltf_json.nodes.length; i++) {
      if (typeof gltf_json.nodes[i].children !== 'undefined') {
        for (let k = 0; k < gltf_json.nodes[i].children.length; k++) {
  ...
  static parseAnimationsFromArrayBuffer(array_buffer) {
    ...
    for (let i = 0; i < gltf_json.animations.length; i++) {
      ...
      for (let k = 0; k < gltf_json.animations[i].channels.length; k++) {
        ...
        if (typeof curr_animation.tracks[node_name] === 'undefined') curr_animation.tracks[node_name] = new AnimationTrack();
        ...
        if (input.componentType != 5126) console.warn('input component type is not a float!');
        ...
        for (var j = 0; j < output.count * output_component_count; j += output_component_count) {
          const value = [];
          for (let l = 0; l < output_component_count; l++) {
  ...
  static _parseAnimationSetFromArrayBuffer(array_buffer) {
    ...
    for (let i = 0; i < animation_nodes.length; i++) {
      ...
      if (typeof nodes_dict[animation_nodes[i].name].translation === 'undefined') nodes_dict[animation_nodes[i].name].translation = [0, 0, 0];
      ...
      if (typeof nodes_dict[animation_nodes[i].name].scale === 'undefined') nodes_dict[animation_nodes[i].name].scale = [0, 0, 0];
    }
